 ...
 * 				order of the list.
 * 
 * Opinion: 	Randomizing the list has no merit. It costs time and cpu to
 * 				shuffle the list and in the worst case scenario all the right numbers
 * 				are put at the end of the list.
 * 
 * Solution 1:  No randomization before returning the list.
 * 
 * Problem 2: 	Creating an arraylist every time a new "solve" method is started
 * 				will create a lot of arraylist that are then redirected into arrays.
 * 				
 * Opinion: 	Since we are removing the randomization of the list these steps are
 * 				unnecessary. We can just use the value of the for-loop to give a value
 * 				to fill in.
 * 
 * Solution 2: 	Removing the array commpletely from the solve method and using the value
 * 				of the for-loop to fill in a value.
 * 
 * @author Daniel
 * @version 2.0
 *
