package com.migapro.sudokusolverV4;
 ...
 * 
 * Problem 3: 	The GUI freezes when the program is solving a sudoku.
 * 				
 * Opinion: 	Freezing GUI's are never good and it a normal use might tink that the program doesn't work
 * 				when the GUI freezes.
 * 
 * Solution: 	By running the solve method on a different thread, the GUI will still be responsive while the
 * 				solve method is running.
 * 				To make sure the thread is not started twice we will also disable the solve button at the start
 * 				of the thread and enable it again at the end.
 * 
 * edits:		1. 	Invoking the method on a different thread. (197)
 * 				2. 	Making the solve button a member of the class. (61 + 119)
 * 				3.	Disabling and enabling the solve button. (225 + 241)
 * 
 * @author Daniel
 * @version 2.0
 *
